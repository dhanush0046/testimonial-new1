// // component/TextTestimonialForm.tsx -> Testingss
"use client"
...
export default function TextTestimonialForm({
  ...
  const handleExtraInfoChange = (
    ...
    if (id === nameField?.id && typeof value === "string") {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (content.length < maxTextCharacters) {
      toast.error(t("errors.minCharacters", { count: maxTextCharacters }));
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col h-full p-3 text-left">
      <div className="flex-grow overflow-y-auto">
        <h2 className="text-2xl text-gray-900 text-center font-bold mb-3">
          {textSubmissionTitle}
        </h2>
        ...
        <div className="space-y-4">
          {collectStarRatings && (
            <div>
              <Label htmlFor="rating">{t("labels.rating")}</Label>
              <span className="text-red-600 ml-1">*</span>
          ...
          <div>
            <Label htmlFor="testimonial">{t("labels.yourTestimonial")}</Label>
            <Textarea
              id="testimonial"
              placeholder={t("placeholders.writeTestimonial", {
            ...
            <p
              className={`text-sm ${
                content.length < maxTextCharacters
                  ? "text-red-500"
                  : "text-green-500"
              }`}
            >
              {content.length}/{maxTextCharacters} {t("labels.characters")}
              {content.length < maxTextCharacters
                ? t("labels.charactersNeeded", {
                    count: maxTextCharacters - content.length,
                  })
                : t("labels.minimumReached")}
            </p>
          </div>
                      ...
                      >
                        <div className="flex items-center justify-center relative w-14 h-14">
                          ...
                          <button
                            ...
                            className="border rounded-md bg-black cursor-pointer overflow-hidden h-full w-full flex items-center justify-center transition hover:scale-105"
                          >
                            <img
                              src={url}
                              alt={t("alt.attached")}
...
// // component/TextTestimonialForm.tsx -> workingsss
...
//           {t("buttons.cancel")}
//         </Button>
//         <Button type="submit" form="testimonial-form" className="border" disabled={isSubmitting}>
//           {isSubmitting ? t("buttons.sending") : t("buttons.send")}        
//         </Button>
//       </div>
//       <Toaster position="top-center" richColors />
...
//   const translatedHeaderTitle = useTranslation(headerTitle || "", selectedLanguage);
//   const translatedTextSubmissionTitle = useTranslation(textSubmissionTitle || "", selectedLanguage);
//   const translatedConsentStatement = useTranslation(consentStatement || "", selectedLanguage);
...
//   //const translatedLabel = useTranslation(label, selectedLanguage);
...
//         <div className="ml-2 text-sm leading-5">
//           <Label htmlFor="permission" className="text-gray-600">
//             {translatedConsentStatement.translatedText}
...
//     <div className="flex flex-col h-full p-3 text-left">
//       <div className="flex-grow overflow-y-auto">
//         <h2 className="text-2xl text-gray-900 text-center font-bold mb-3">{translatedTextSubmissionTitle.translatedText}</h2>
//         <div className="text-center mb-6">
//           {logo && (
//             <div className="w-24 h-24 mx-auto mb-6 relative overflow-hidden inline-flex justify-center rounded-md">
//               <Image
...
//             </div>
//           )}
//           <h3 className="font-bold text-xl text-gray-900">{translatedHeaderTitle.translatedText}</h3>
//         </div>
...
//               <li key={question.id}>{question.translatedContent.translatedText}</li>
//             ))}
//           </ul>
//         </div>
